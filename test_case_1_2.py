...
def case_1_2(self, full_screen):
    ...
    self.page.loger('\n Запуск Тест кейс № 1_2 tvweb_new-1_2: Проверка воспроизведения прямой эфир \n')

    time.sleep(2)
    self.driver.find_element_by_xpath('.//a[@href="/on-air"]').click()
    self.page.loger('Шаг 1 Осуществлен переход в раздел эфир')
    time.sleep(3)

    target = self.driver.find_element_by_xpath('.//h1[@class="heading caption__heading"]')  # скролл до плеера прямой эфир
    target.location_once_scrolled_into_view  # скролл

    self.driver.find_element_by_xpath('.//div[@class="video-player"]').click()
    self.page.loger('Шаг 2 Клик на воспроизведение плеера')
    time.sleep(10) # Долгое ожидание для проигрывания рекламы

    # Добавить проверку на воспроизведение рекламы, т.к. цепляется за промежуток между рекламами или ещё за что-то
    # убедиться, что реклама закончилась

    self.driver.waitForElementVisible('.//div[@class="video-js vjs-default-skin vjs-fill vjs-big-play-centered vjs-matchtv-skin vjs-controls-enabled vjs-workinghover vjs-videojs-smart-tracking vjs-videojs-heartbeat-tns-counter vjs-has-started vjs-live player-dimensions vjs-playing vjs-user-inactive"]')
    self.page.loger('Прямой эфир воспроизводится')
    time.sleep(2)

    self.driver.find_element_by_xpath('.//div[@class="tv-schedule-line tv-schedule-line_theme_dark"]')
    self.page.loger('Шаг 3 Наличие полоски программы передач подтверждено')
    time.sleep(2)

    self.driver.find_element_by_xpath('.//h3[@class="heading caption__heading"][contains(., "ТОП-ТРАНСЛЯЦИИ")]')
    self.page.loger('Шаг 4 Наличие заголовка Топ трансляции подтверждено')  # добавить проверку не только заголовка, но и блока
